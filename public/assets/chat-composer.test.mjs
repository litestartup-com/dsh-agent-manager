...
import { modelKey, shortPath, accessOptions, sendPolicy } from './chat-composer.js'
...
test('债务 F1: accessOptions 第三档随 fullAccess 解锁状态变化', () => {
  ...
  assert.deepEqual(unlocked[2], { value: 'danger-full-access', label: '全量访问', danger: true })
  const locked = accessOptions({ fullAccess: false })
  assert.equal(locked.length, 3)
  assert.deepEqual(locked[2], { value: 'danger-full-access', label: '全量访问 · 节点未开启', danger: true, locked: true })
})
