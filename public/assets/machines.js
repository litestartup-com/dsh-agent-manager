...
/**
 * 机器（agent）行：在线点 / 吊销态 / 待执行指令数。
 * @param {{ id: string, hostname: string, os: string, arch: string, nodeVersion: string, joinedAt: number, online: boolean, revoked: boolean, pendingCommands: number }} m
 * @returns {string}
 */
export const machineRowHtml = (m) => {
  ...
  return `<div class="node-row" data-machine-row="${esc(m.id)}">
    <div class="node-main">
      <div class="node-title"><span class="dot ${dot}"></span>${esc(m.hostname)} <span class="muted">· ${esc(m.os)}/${esc(m.arch)} · node ${esc(m.nodeVersion)}</span></div>
