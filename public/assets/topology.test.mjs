...
test('UI 收尾 C: 形态 tag——远端/容器/宿主机/外管', () => {
  assert.equal(formTag(node({ host: 'agent-abc' })), 'agent 远端')
  assert.equal(formTag(node({ image: 'ohdsh:0.1.5' })), '容器工蜂')
  assert.equal(formTag(node({})), '宿主机进程')
  assert.equal(formTag(node({ managed: false })), '外管')
})
...
test('UI 收尾 C: manager 卡片——版本/监听面/部署形态/计数', () => {
  ...
  assert.ok(html.includes('裸机部署'))
  assert.ok(html.includes('3 台机器 · 5 个节点'))
  assert.ok(managerCardHtml({ managerVersion: '1.1.2', origin: 'x', containerForm: true, machineCount: 0, nodeCount: 0 }).includes('容器部署'))
})
...
test('UI 收尾 C: 机器卡片——在线点/待更新/吊销/指标徽标', () => {
  ...
  assert.ok(!on.includes('待更新'), '同版本无徽标')
  const stale = machineCardHtml(machine({ agentVersion: '1.0.0' }), '1.1.2')
  assert.ok(stale.includes('待更新'), '旧 agent 版本显示待更新')
  ...
  assert.ok(revoked.includes('已吊销'))
  ...
  assert.ok(metrics.includes('内存 50%'))
  assert.ok(metrics.includes('磁盘 60%'))
})
...
test('UI 收尾 C: 节点卡片——状态点/工作区/版本/漂移/主机映射', () => {
  ...
  assert.ok(!html.includes('版本漂移'))
  const warn = nodeCardHtml(node({ dshCompatible: false, dshDrift: true }), new Map())
  assert.ok(warn.includes('版本告警') && warn.includes('版本漂移'))
  const remote = nodeCardHtml(node({ host: 'agent-abc' }), new Map([['agent-abc', 'srv-b']]))
  assert.ok(remote.includes('agent 远端'))
  assert.ok(remote.includes('主机 srv-b'), '主机名映射')
